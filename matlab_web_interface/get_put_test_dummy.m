...
for i=1:size(tasks)
  ...
  if ~exist(task_full_file, 'file')
    ...
    dummy = 255*ones(300,300,3);
    dummy2 = insertText(dummy,[0 0],...
      sprintf('%s\n%s\n%s',tasks(i,1),tasks(i,2),tasks(i,3)), 'FontSize', 42);
    imwrite(dummy2,task_full_file);
    %%Montage(tasks(i,2),task_full_file);
